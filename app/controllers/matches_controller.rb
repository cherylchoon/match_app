class MatchesController < ApplicationController
  def create
    ...
    if @match_one.preference.nil?
      flash[:errors] = ["You must set at least a few preferences to retrieve matches!"]
    end
    @possible_matches = User.where('gender=? AND id != ?', @match_one.preference.gender, @match_one.id)
    if @match_one.preference
      ...
      end #ends m.profile statement
    end  #ends possible match loop
  end #ends if @match_one.preference statement
    redirect_to root_path and return
  end

  def show
    @match = Match.where("match_one_id=? AND is_match=?", session[:user_id], false).order(score: :desc)
    @users = User.all
    @time_now = Time.now
  end
